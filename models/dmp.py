...
class CanonicalSystem:
    ...
    """
    def __init__(self, dt=0.01, ax=1.0, run_time=1, device='cpu'):
        """
...
class DMPCore(nn.Module):
    ...
    Core part about DMP.
    """
    def __init__(self, n_bfs, cs, dt=0.01, run_time=1.0, ay=None,
    ...
    def torque_step(self, f, tau=1.0, error=0.0, external_force=None):
        """
    ...
    def torque_rollout(self, f, y0, goal):
        """
        ...
        self.reset()
        y_track = np.zeros(time_step)
        dy_track = np.zeros(time_step)
        ddy_track = np.zeros(time_step)
        # generate an initial trajectory
        for t in range(time_step):
            y_track[t], dy_track[t], ddy_track[t] = self.torque_step(f[t])

        # scale the initial trajectory based on the goal
        scale = (goal - y0) / (y_track[-1] - y_track[0])
        f = f * scale
...
class SingleDMP(DMPCore):
    ...
    def __init__(self, n_bfs, cs, dt=0.01, ay=None,
                 by=None, run_time=1.0, w=None, device='cpu'):
        """
